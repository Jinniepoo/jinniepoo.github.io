    ...
    <div class="modal-feature-section">
        ...
        <div class="modal-feature-row">
            <div class="text-content">
                ...
                <p><strong>문제 해결 방법:</strong> <strong>Unity NavMeshAgent</strong>와 **Mouse Raycast**를 결합하여 이동 로직을 설계하고, **Reticle UI**를 통해 피드백을 제공했습니다. </p>
                <p><strong>해결 과정:</strong> 마우스 클릭 시 **Mouse Raycast**를 바닥에 쏘아 목표 지점의 3D 좌표를 획득합니다. **NavMeshAgent** 컴포넌트의 \`SetDestination\` 함수를 사용하여 캐릭터를 해당 좌표로 이동시키며, 이동 중에는 애니메이션 컨트롤러를 통해 \`MoveState\`로 전환하고 목표 지점에 Reticle UI를 표시합니다.</p>
            </div>
            <img src="Packages/GitImages/ClickUI.gif" alt="탑다운 마우스 Raycast 기반 캐릭터 이동 GIF" />
        </div>
    </div>
    ...
    <div class="modal-feature-section">
        ...
        <div class="modal-feature-row reverse">
            <div class="text-content">
                ...
                <p><strong>문제 해결 방법:</strong> **FSM 패턴**을 핵심 구조로 채택하여 상태별 로직을 분리하고, **NavMeshAgent**를 활용하여 몬스터의 순찰/추적 경로를 구현했습니다.</p>
                <p><strong>해결 과정:</strong> **IdleState, MoveState, AttackState, DeadState** 등 상태 클래스를 정의하여 공통 로직을 관리했습니다. 몬스터는 **Waypoint**를 따라 순찰하다가, **시야 범위(Detection Range)** 내에 플레이어가 감지되면 추적으로 전환합니다. 공격 거리에 도달하면 **AttackState**로 전환하여 거리 기반 공격을 수행하고 이펙트를 연동했습니다.</p>
            </div>
            <img src="Packages/GitImages/MonsterAtt.gif" alt="FSM 기반 몬스터 공격 및 전투 GIF" />
        </div>
    </div>
    ...
    <div class="modal-feature-section">
        ...
        <div class="modal-feature-row">
            <div class="text-content">
                ...
                <p><strong>문제 해결 방법:</strong> **Blender**에서 장비 파츠를 직접 제작하고, Unity에서 기존 **캐릭터의 Armature(Bone)**를 재활용하여 부착하는 방식으로 문제를 해결했습니다.</p>
                <p><strong>해결 과정:</strong> Blender에서 신규 장비 모델을 제작 후, 기존 캐릭터 모델의 **Armature 정보를 유지**하며 장비를 바인딩했습니다. 게임 내에서 장비 장착 시, 해당 **Skinned Mesh Prefab**을 캐릭터 Armature의 적절한 위치에 인스턴스화(Instantiate)하여 교체를 구현했습니다. 또한 무기 Mesh 등은 Armature에 수작업으로 연결하여 동작 시 자연스러움을 확보했습니다.</p>
            </div>
            <img src="Packages/GitImages/EquipSkinned.gif" alt="Skinned Mesh 장비 교체 GIF" />
        </div>
    </div>
    ...
    <div class="modal-feature-section">
        ...
        <div class="modal-feature-row reverse">
            <div class="text-content">
                ...
                <p><strong>문제 해결 방법:</strong> **Scriptable Object**를 사용하여 아이템 데이터를 에셋으로 관리하고, 충돌 기반의 **자동 습득 로직** 및 **우클릭 소비 로직**을 구현했습니다.</p>
            ...
            <img src="Packages/GitImages/LootItems.gif" alt="아이템 습득 및 소비 GIF" />
        </div>
    </div>
    ...
    <div class="modal-feature-section">
        ...
        <div class="modal-feature-row">
            <div class="text-content">
                ...
                <p><strong>문제 해결 방법:</strong> 마우스 **Raycast**를 활용한 NPC **Targeting** 기능과 **거리 감지 로직**을 결합하여 대화 시작 조건을 구현했습니다.</p>
            ...
            <img src="Packages/GitImages/NPC.gif" alt="NPC 대화 시스템 GIF" />
        </div>
    </div>
            ...
            <div class="modal-feature-section">
                ...
                <div class="modal-feature-row">
                    <div class="text-content">
                        ...
                        <p><strong>문제 해결 방법:</strong> **Grid Data Array**를 통해 그리드 상태를 관리하고, 블록의 모양을 정의하는 **Shape Data**와 대조하는 **\`CanPlaceShape\`** 로직을 핵심으로 구현했습니다.</p>
            ...
            <div class="modal-feature-section">
                ...
                <div class="modal-feature-row">
                    <div class="text-content">
                        ...
                        <p><strong>문제 해결 방법:</strong> **ShapeStorage** 클래스로 **Queue 방식**의 블록 관리 시스템을 구축하고, **RequestNewShape** 시스템을 통해 블록 큐를 리셋하는 로직을 구현했습니다.</p>
            ...
            <div class="modal-feature-section">
                ...
                <div class="modal-feature-row">
                    <div class="text-content">
                        ...
                        <p><strong>문제 해결 방법:</strong> **BonusManager**를 활용하여 **Chain Count** 시스템을 추적하고, 제거된 라인 수 기반의 **점수 승수**를 적용하는 로직을 구현했습니다.</p>
                        <p><strong>해결 과정:</strong> 라인 제거 성공 시마다 **Chain Count**를 증가시키고, 기본 점수에 승수를 적용하여 보너스 점수를 차등 지급했습니다. 한 번의 배치로 여러 줄이 제거되면 라인 수에 비례한 **Multi-Clear Bonus**를 합산하여 최종 점수를 산출합니다.</p>
                </div>
            </div>
            ...
            <div class="modal-feature-section">
                ...
                <div class="modal-feature-row">
                    <div class="text-content">
                        ...
                        <p><strong>문제 정의:</strong> 사용자의 최고 점수를 로컬 환경에 **영구적으로 저장**하고 불러와야 하며, 데이터의 안정성을 확보해야 했습니다.</p>
                        <p><strong>문제 해결 방법:</strong> 간단한 데이터 저장은 **Unity PlayerPrefs**를 사용하고, 중요 데이터인 최고 점수는 **Binary Serialization**을 조합하여 데이터 지속성(Persistence)을 구현했습니다.</p>
                        <p><strong>해결 과정:</strong> \`BinaryData\` 유틸리티 클래스를 통해 최고 점수 데이터를 **바이너리 파일**로 직렬화하여 영구 저장합니다. 게임 시작 시 파일을 로드하고, 게임 중 실시간 점수를 PlayerPrefs에 임시 저장 후 최고 점수 갱신 시 바이너리 파일에 최종 저장하여 데이터 변조 위험을 낮추었습니다.</p>
                </div>
            </div>
        </div>`
    }
